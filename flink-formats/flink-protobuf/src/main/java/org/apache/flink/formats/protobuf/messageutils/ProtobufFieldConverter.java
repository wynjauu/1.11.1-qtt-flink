...
public abstract class ProtobufFieldConverter implements Serializable {
	...
	private void validateTypeInformation() {
		...
		Descriptors.FieldDescriptor.JavaType[] javaTypes = new Descriptors.FieldDescriptor.JavaType[fieldNames.length];
		for (int i = 0; i < fieldNames.length; i++) {
			String fieldName = fieldNames[i];
			Descriptors.FieldDescriptor fieldDescriptor = FIELD_DESCRIPTORS.get(fieldName);
			if (fieldDescriptor == null) {
				throw new ProtobufException(String.format("field '%s' cannot be found in protobuf schema (%s)", fieldName, PROTOBUF_SCHEMA));
			}
