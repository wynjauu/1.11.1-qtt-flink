...
public class ProtobufRowFormatFactory extends TableFormatFactoryBase<Row> implements DeserializationSchemaFactory<Row>, SerializationSchemaFactory<Row> {
	...
	public DeserializationSchema<Row> createDeserializationSchema(Map<String, String> properties) {
		...
		ProtoBufRowDeserializationSchema.Builder builder = ProtoBufRowDeserializationSchema.builder();
		builder.setTypeInfo(createTypeInformation(descriptorProperties));
		builder.setMessageVersion(descriptorProperties.getString(FORMAT_QUTOUTIAO_MESSAGE_VERSION));
...
//		return rowTypeInfo ;
		final boolean deriveSchema = descriptorProperties.getOptionalBoolean(FORMAT_DERIVE_SCHEMA).orElse(true);
		final String messageVersion = descriptorProperties.getString(FORMAT_QUTOUTIAO_MESSAGE_VERSION);
		if (deriveSchema) {
