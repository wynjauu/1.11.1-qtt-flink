...
public abstract class KafkaTableSourceSinkFactoryBase
    ...
    public List<String> supportedProperties() {
        ...
        // 延迟消费的参数
        properties.add(DELAY_OPEN);
        properties.add(DELAY_SECOND);
        properties.add(DELAY_EVENTTIME_FIELD_INDEX);
        properties.add(DELAY_CACHE_SIZE);
        properties.add(KEYBY_INDEX);
        properties.add(KEYBY_FIELD);
    ...
    private Properties getKafkaProperties(DescriptorProperties descriptorProperties) {
        ...
        Boolean needDelay = descriptorProperties.getOptionalBoolean(DELAY_OPEN).orElse(false);
        long delaySecond = descriptorProperties.getOptionalLong(DELAY_SECOND).orElse(0L);
        int eventTimeFieldIndex =
                descriptorProperties.getOptionalInt(DELAY_EVENTTIME_FIELD_INDEX).orElse(0);
        int delayCacheSize =
                descriptorProperties
                        .getOptionalInt(DELAY_CACHE_SIZE)
                        .orElse(DEFAULT_DELAY_CACHE_SIZE);
        int parallelism = descriptorProperties.getOptionalInt(PARALLELISM).orElse(0);

        kafkaProperties.setProperty(DELAY_OPEN, String.valueOf(needDelay));
        kafkaProperties.setProperty(DELAY_SECOND, String.valueOf(delaySecond));
        kafkaProperties.setProperty(
                DELAY_EVENTTIME_FIELD_INDEX, String.valueOf(eventTimeFieldIndex));
        kafkaProperties.setProperty(DELAY_CACHE_SIZE, String.valueOf(delayCacheSize));
        kafkaProperties.setProperty(PARALLELISM, String.valueOf(parallelism));
        kafkaProperties.setProperty(
                KEYBY_INDEX,
                String.valueOf(descriptorProperties.getOptionalInt(KEYBY_INDEX).orElse(-1)));
        kafkaProperties.setProperty(
                KEYBY_FIELD, descriptorProperties.getOptionalString(KEYBY_FIELD).orElse(""));
