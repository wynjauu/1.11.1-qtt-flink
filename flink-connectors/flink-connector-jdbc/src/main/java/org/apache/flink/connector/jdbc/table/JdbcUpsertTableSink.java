...
public class JdbcUpsertTableSink implements UpsertStreamTableSink<Row> {
	...
	public DataStreamSink<?> consumeDataStream(DataStream<Tuple2<Boolean, Row>> dataStream) {
		DataStreamSink<Tuple2<Boolean, Row>> streamSink = dataStream
			.addSink(new GenericJdbcSinkFunction<>(newFormat()));
		if (parallelism > 0) {
			streamSink.setParallelism(parallelism);
		} else {
			streamSink.setParallelism(dataStream.getParallelism());
		}
		return streamSink.name(TableConnectorUtils.generateRuntimeName(this.getClass(), schema.getFieldNames()));
	}
	...
	public TableSink<Tuple2<Boolean, Row>> configure(String[] fieldNames, TypeInformation<?>[] fieldTypes) {
		...
		JdbcUpsertTableSink copy = new JdbcUpsertTableSink(schema, options, flushMaxSize, flushIntervalMills, maxRetryTime,parallelism);
	...
	public static class Builder {
		...
		public JdbcUpsertTableSink build() {
			...
			return new JdbcUpsertTableSink(schema, options, flushMaxSize, flushIntervalMills, maxRetryTimes,parallelism);
		}
	}
}
