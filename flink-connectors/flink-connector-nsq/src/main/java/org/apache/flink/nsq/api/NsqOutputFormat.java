package org.apache.flink.nsq.api;
...
public class NsqOutputFormat<T> extends RichOutputFormat<T> {
	...
	public void writeRecord(T value) throws IOException {
		...
		if (value instanceof Tuple2) {
			...
			BinaryMapData binaryMapData = (BinaryMapData)RowData.createFieldGetter(dataTypes[0].getLogicalType(), 0).getFieldOrNull(rowData);
			msg = objectMapper.writeValueAsBytes(getMapValue(binaryMapData,dataTypes[0].getLogicalType()));
		} else {
			LOG.error("record type is unsupported.record is {}", value);
			throw new FlinkRuntimeException(String.format("record type is unsupported.record is %s", value));
		}
		...
		if (failFast) {
			throw new RuntimeException("Send nsq failed frequently, five time per second, do fail fast.");
		}
	}

	private Map<String,String> getMapValue(BinaryMapData binaryMapData, LogicalType logicalType) {
		Map<?, ?> tmpMap = binaryMapData.toJavaMap(logicalType.getChildren().get(0), logicalType.getChildren().get(1));
