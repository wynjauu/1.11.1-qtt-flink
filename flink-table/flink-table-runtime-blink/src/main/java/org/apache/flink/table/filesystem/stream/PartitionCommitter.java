...
public class PartitionCommitter extends AbstractStreamOperator<Void>
	...
	private final FileSystemWithUserFactory fsWithUserFactory;
	...
	public PartitionCommitter(
		...
		Configuration conf) {
		this(locationPath, tableIdentifier, partitionKeys, metaStoreFactory, fsFactory, null, conf);
	}
	...
	public PartitionCommitter(
		...
		this.fsWithUserFactory = fsWithUserFactory;
		this.conf = conf;
	...
	public void initializeState(StateInitializationContext context) throws Exception {
		...
		this.policies = PartitionCommitPolicy.createPolicyChain(
			...
			() -> {
				String hadoopUser = conf.get(HADOOP_USER);
				try {
					if (StringUtils.isNotEmpty(hadoopUser)) {
						return fsWithUserFactory.create(locationPath.toUri(), hadoopUser);
					} else {
