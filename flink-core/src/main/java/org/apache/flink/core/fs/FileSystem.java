...
public abstract class FileSystem {
	...
	public static FileSystem getWithUser(URI uri, String user) throws IOException {
		return FileSystemSafetyNet.wrapWithSafetyNetWhenActivated(getUnguardedFileSystem(uri, user));
	}

	@Internal
	public static FileSystem getUnguardedFileSystem(final URI fsUri) throws IOException {
		return getUnguardedFileSystem(fsUri, "");
	}

	public static FileSystem getUnguardedFileSystem(final URI fsUri, final String user) throws IOException {
			...
			if (factory != null) {
				...
				try {
					if(StringUtils.isNotEmpty(user)) {
						fs = FALLBACK_FACTORY.create(uri, user);
					} else {
