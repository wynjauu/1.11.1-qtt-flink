...
public interface FileSystemFactory extends Plugin {
	 ...
	 * @throws IOException
	 */
	default FileSystem create(URI fsUri, String user) throws IOException {
		throw new IOException(String.format("Unsupport create filesystem with user in [%s] schema, uri [%s], user [%s]",
			fsUri.getScheme(), fsUri.toString(), user));
	}
}
