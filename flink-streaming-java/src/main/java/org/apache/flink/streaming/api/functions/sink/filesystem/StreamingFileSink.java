...
public class StreamingFileSink<IN>
	...
	public abstract static class BucketsBuilder<IN, BucketID, T extends BucketsBuilder<IN, BucketID, T>> implements Serializable {
		...
		@Internal
		public BucketWriter<IN, BucketID> createBucketWriter(final String hadoopUser) throws IOException{
			throw new RuntimeException("Unsupport create buckets writer with hadoop user.");
		};

		@Internal
		public abstract Buckets<IN, BucketID> createBuckets(final int subtaskIndex) throws IOException;

		public Buckets<IN, BucketID> createBuckets(final int subtaskIndex, final String hadoopUser) throws IOException {
			throw new RuntimeException("Unsupport create buckets function with hadoop user.");
		}
	}
	...
	public static class RowFormatBuilder<IN, BucketID, T extends RowFormatBuilder<IN, BucketID, T>> extends StreamingFileSink.BucketsBuilder<IN, BucketID, T> {
		...
		@Internal
		@Override
		public BucketWriter<IN, BucketID> createBucketWriter(String hadoopUser) throws IOException {
			return new RowWiseBucketWriter<>(FileSystem.getWithUser(basePath.toUri(), hadoopUser).createRecoverableWriter(), encoder);
		}
		...
		@Override
		public Buckets<IN, BucketID> createBuckets(int subtaskIndex, String hadoopUser) throws IOException {
			return new Buckets<>(
				basePath,
				bucketAssigner,
				bucketFactory,
				createBucketWriter(hadoopUser),
				rollingPolicy,
				subtaskIndex,
				outputFileConfig);
		}
	...
	public static class BulkFormatBuilder<IN, BucketID, T extends BulkFormatBuilder<IN, BucketID, T>> extends StreamingFileSink.BucketsBuilder<IN, BucketID, T> {
		...
		@Override
		public BucketWriter<IN, BucketID> createBucketWriter(String hadoopUser) throws IOException {
			return new BulkBucketWriter<>(FileSystem.getWithUser(basePath.toUri(), hadoopUser).createRecoverableWriter(), writerFactory);
		}
